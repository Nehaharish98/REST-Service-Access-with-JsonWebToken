...
missing_vars = [v for v in ["AUTH_URL", "TRANSPORT_URL", "CLIENT_ID", "CLIENT_SECRET", "SCOPE"] if not globals()[v]]
if missing_vars:
    log_warn("set the values in the .env file as shown in .env.example: {', '.join(missing_vars)}")
    sys.exit(1)
...
# table rows of the transport list    
    ...
    log_info(f"{len(table_rows)} transport data is fetched at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}.")
